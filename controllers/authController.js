...
module.exports = {
        ...
        try {
          ...
          db.query(`insert into users set ?`, data, (err) => {
            try {
              if (err) throw { msg: "Server Error" };
              db.query(
                `select id, nama_lengkap, email, jenis_kelamin, role, kelurahan, kecamatan, kode_pos, nomor_telepon, nik from users where email='${data.email}'`,
                (err, selectResult) => {
                  try {
                    if (err) throw { msg: "Server Error" };
  ...
  onLogin: (req, res) => {
    ...
    const hashedPassword = crypto
      ...
      .digest("hex");
    db.query(
      `select id, nama_lengkap, email, jenis_kelamin, role, kelurahan, kecamatan, kode_pos, nomor_telepon, nik from users where email='${email}' and kata_sandi='${hashedPassword}'`,
      (err, selectResult) => {
        try {
          if (err) throw { msg: "Server Error" };
